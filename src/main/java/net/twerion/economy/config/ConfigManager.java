...
public class ConfigManager {
	 ...
	 private Main plugin;
	     ...
	     */
	    public ConfigHandler getNewConfig(String filePath, String[] header) {

	        File file = this.getConfigFile(filePath + ".yml");
	        ConfigHandler config;

	        if (!file.exists()) {
	            this.prepareFile(filePath + ".yml");

	            if (header != null && header.length != 0) {
	                this.setHeader(file, header);
	            }
	            config = new ConfigHandler(this.getConfigContent(filePath), file, this.getCommentsNum(file), true, plugin);
	        } else {
	            config = new ConfigHandler(this.getConfigContent(filePath), file, this.getCommentsNum(file), false, plugin);
	        }
	     ...
	     * @return - New SimpleConfig
	     */
	    public ConfigHandler getNewConfig(String filePath) {
	    ...
	    private File getConfigFile(String file) {
	        ...
	        if (file.contains("/")) {
	            ...
	            if (file.startsWith("/")) {
	                configFile = new File(Bukkit.getServer().getPluginManager().getPlugin(plname).getDataFolder() + file.replace("/", File.separator));
	            } else {
	                configFile = new File(Bukkit.getServer().getPluginManager().getPlugin(plname).getDataFolder() + File.separator + file.replace("/", File.separator));
	            }

	        } else {
	            configFile = new File(Bukkit.getServer().getPluginManager().getPlugin(plname).getDataFolder(), file);
	        }
	    ...
	    public void prepareFile(String filePath, String resource) {
	        ...
	        try {
	            ...
	            if (resource != null && !resource.isEmpty()) {
	                this.copyResource(Bukkit.getServer().getPluginManager().getPlugin(plname).getResource(resource), file);
	            }
	        ...
	        try {
	            ...
	            while ((currentLine = reader.readLine()) != null) {
	                ...
	                if (currentLine.startsWith("#")) {
	                    addLine = currentLine.replace("#", pluginName + "_COMMENT_" + commentNum + ":");
	    ...
	    public String getPluginName() {
	        return Bukkit.getServer().getPluginManager().getPlugin(plname).getDescription().getName();
	    }
